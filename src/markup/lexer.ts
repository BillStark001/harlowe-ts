...
export class Token implements TokenData {
  ...
  addChild(tokenData: TokenData) {
    ...
    childToken.end = tokenData.text ? index + tokenData.text.length : -1;
    /**
  ...
  lastChildEnd() {
    ...
    */
      Math.max(0, this.text?.indexOf(this.innerText) ?? -1);
  }
  ...
  nearestTokenAt(index: number) {
    ...
    /**
      ...
      */
    if (this.children) {
      return this.children.reduce((prevValue: Token | null, child) => {
        return (
          prevValue ||
  ...
  /**
    ...
    */
  everyLeaf<T = unknown>(fn: (self: Token) => T): boolean {
...
/**
  ...
  */
function lex(parentToken: Token, fold?: boolean) {
  ...
  while (index < endIndex) {
    ...
    for (; i < l; i += 1) {
      ...
      /**
        ...
        */
      let folded = false;
      if (isMatchingBack && frontTokenStack) {
        /**
...
/**
    ...
    children array.
  */
function foldTokens(parentToken: Token, backToken: Token, frontToken: Token) {
  /**
